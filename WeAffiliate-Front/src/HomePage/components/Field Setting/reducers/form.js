...
function addField(state, field) {
  ...
  const name = `Untitled ${state.currentIndex}`;
  const _slug = slugify(name);
  state.schema.id.push(field.label)
  state.schema.properties[_slug] = {...field.jsonSchema,title:name,id:field.id,showCategories:[]};
