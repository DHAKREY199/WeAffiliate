...
function getClientID(widgetID){
  console.log(typeof widgetID)
  addElement(widgetID);
}

async function addElement (widgetID) {

        // creation of feedback form container div
         let result ;
        await $.ajax({
        url: `http://localhost:5000/api/form/getClientButtonSetting?widgetID=${widgetID}`,
        type: 'GET',
        dataType: 'json', // added data type
        success: function(res) {
             console.log(res);
            // alert(res);
            result = res.result[0].buttonSetting;
        }
    });
    console.log(result)
        ...
        const srcAddr = `http://localhost:3000/clientForm/${widgetID}`;
        ...
        button.setAttribute("style", `color:${result.textColor}; margin-top: 200px;font-weight: bold;padding: 0 20px;background: ${result.backgroundColor};border: ${result.borderColor};float: right; vertical-align:top; margin-right: -45px;width:120px;height:30px;transform:rotate(7deg);-ms-transform:rotate(270deg);-moz-transform:rotate(270deg);-webkit-transform:rotate(270deg);-o-transform:rotate(270deg); `);
